...
- (id)initWithFileHandle:(NSFileHandle *)fh delegate:(id)initDelegate
{
    if(self = [super init]) {
		...
		NSNotificationCenter *nc = [NSNotificationCenter defaultCenter];
		[nc addObserver:self
			   selector:@selector(dataReceivedNotifications:)
...
- (void)dataReceivedNotifications:(NSNotification *)notification
{
    NSLog(@"cocoaservernot: %@",notification);
