...
-(void)setupButtons
{
  ...
  //  NSLog(@"parent of _removebackup: %@",); 
    ...
    //[_removeButton release];
    if([_information isInstalled] && ![_information isHidden])
    {
        _hideButton = [SMButtonControl actionButtonWithTitle:@"Hide" subtitle:@"" badge:nil];
        _isHidden=NO;
    }
    else if([_information isInstalled] && [_information isHidden])
    {
        _hideButton = [SMButtonControl actionButtonWithTitle:@"Show" subtitle:@"" badge:nil];
        _isHidden=YES;
    }
    else 
    {
        _hideButton = [SMButtonControl dashedActionButtonWithTitle:@"Cannot hide" subtitle:@"not installed" selectable:NO];
    }
    frame.origin.x=frame.origin.x-frame.size.height*1.2f;
    [_hideButton setFrame:frame];
...
//		remoteAction ++;
	...
	switch (remoteAction)
	{
		case kBREventRemoteActionPlay:  // tap play
        {
            ...
            else if([_hideButton isFocused] && [_hideButton isEnabled])
            {
                if(!_isHidden)
                {
