...
{
    return [[[NSFileManager defaultManager]currentDirectoryPath] stringByAppendingPathComponent:@"hello"];
}
-(NSString *)imagesPath
{
     return [[[NSFileManager defaultManager]currentDirectoryPath] stringByAppendingPathComponent:@"Images"];
}
-(int)loadPlugins
{
    NSLog(@"starting");
    ...
    if([man fileExistsAtPath:[self trustedPlistPath]] && [[TrustedDict allKeys] count]>1)
		[man removeFileAtPath:[self trustedPlistPath] handler:nil];
	[TrustedDict writeToFile:[self trustedPlistPath] atomically:YES];
    //    if (_checkImages) {
    ...
    //return [self trustedPlistPath];//[SMGeneralMethods checkUpdatesPath];
    return 0;
}
-(void)getImages:(NSDictionary *)TrustedDict
{
	...
	{
        if(![man fileExistsAtPath:[[self imagesPath] stringByAppendingPathComponent:[name stringByAppendingPathExtension:[ImageURL pathExtension]]]] || _updateImages)
        {
            ...
            if([ImageURL length] == 0)
            {
                if([name isEqualToString:@"ATVFiles"])
                {
                    ...
                    [imageData writeToFile:[[self imagesPath] stringByAppendingPathComponent:[name stringByAppendingPathExtension:[ImageURL pathExtension]]] atomically:YES];
            ...
            if([ImageURL length]!=0 )
            {
                ...
                NSData *imageData = [NSData dataWithContentsOfURL:[NSURL URLWithString:ImageURL]];
                if([man fileExistsAtPath:[[self imagesPath] stringByAppendingPathComponent:[name stringByAppendingPathExtension:[ImageURL pathExtension]]]])
                {
                    [man removeFileAtPath:[[self imagesPath] stringByAppendingPathComponent:[name stringByAppendingPathExtension:[ImageURL pathExtension]]] handler:nil];
                }
                [imageData writeToFile:[[self imagesPath] stringByAppendingPathComponent:[name stringByAppendingPathExtension:[ImageURL pathExtension]]] atomically:YES];
...
{
    NSString *path=[[self trustedPlistPath] stringByAppendingPathComponent:@"install.log"];
	NSString * thelog2 = [[[[NSString alloc] initWithContentsOfFile:path] stringByAppendingString:@"\n"] stringByAppendingString:str];
	[thelog2 writeToFile:path atomically:YES];
}
@end
