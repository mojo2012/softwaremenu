...
@implementation SoftwareManualInfo
-(id)init{
	//NSLog(@"init");
...
-(id)initWithIdentifier:(NSString *)initId
{
	...
	[self setListTitle: @"Manage Untrusted Specific"];
	...
	{
		tempdict = [NSDictionary dictionaryWithContentsOfFile:[@"~/Library/Application Support/SoftwareMenu/unTrusted/untrusted.plist" stringByExpandingTildeInPath]];
		[untrusted addEntriesFromDictionary:tempdict];
		//NSLog(@"adding from temp dict untrusted");
	}
	NSDictionary *theoption = [[NSDictionary alloc]initWithDictionary:[tempdict valueForKey:initId]] ;
	
	_items = [[NSMutableArray alloc] initWithObjects:nil];
	_options = [[NSMutableArray alloc] initWithObjects:nil];
	//int counter;
	id item1 = [[BRTextMenuItemLayer alloc] init];
	[_options addObject:[[NSArray alloc] initWithObjects:@"Add",[theoption valueForKey:@"name"],nil]];
	[item1 setTitle:[theoption valueForKey:@"name"]];
	[_items addObject:item1];
	id item2 =[[BRTextMenuItemLayer alloc] init];
	[_options addObject:[[NSArray alloc] initWithObjects:@"Address",[theoption valueForKey:@"theURL"],nil]];
	[item2 setTitle:[theoption valueForKey:@"theURL"]];
	[_items addObject:item2];
	id item3 =[BRTextMenuItemLayer networkMenuItem];
	[_options addObject:@"Test"];
	[item3 setTitle:@"Test File"];
	...
	[list addDividerAtIndex:0 withLabel:@"Name"];
	[list addDividerAtIndex:1 withLabel:@"Address"];
	[list addDividerAtIndex:2 withLabel:@"Check if it actually exists"];
