...
-(void)itemSelected:(long)row
{
	...
	switch([[settingNumberType objectAtIndex:row] intValue])
	{
        ...
        {
			id controller = [SMFPasscodeController passcodeWithTitle:BRLocalizedString(@"Set Time Per Slide",@"Set Time Per Slide")
        ...
        {
			id controller = [SMFPasscodeController passcodeWithTitle:BRLocalizedString(@"Set Screensaver Time Out",@"Set Screensaver Time Out")
    ...
    switch (_returnType) {
        case kSMSSSTimeOut:
            if ([[sender stringValue]intValue]==0) {
                [[ATVSettingsFacade singleton] setScreenSaverEnabled:NO];
        ...
        case kSMSSSRotation:
            ...
            break;
        case kSMSSSSlideTime:
            [SMPreferences setScreensaverSecondsPerSlide:[[sender stringValue]intValue]];
            break;
        default:
            break;
    }
