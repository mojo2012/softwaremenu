...
#define PLUGIN_PREF_ORDER @""

@implementation SMApplianceDictionary
+(SMApplianceDictionary *)appDictionaryWithDictionary:(NSDictionary *)dict
{
    return [[SMApplianceDictionary alloc]initWithDictionary:dict];
}
...
-(BOOL)isInstalled
{
   	NSString *frapPath= [ATV_PLUGIN_PATH stringByAppendingPathComponent:[[self name] stringByAppendingPathExtension:@"frappliance"]];
...
-(BOOL)isHidden
{
    ...
    if ([[infoDict objectForKey:kSMFApplianceOrderKey] floatValue]<0)
        return YES;
    return NO;
}
-(BOOL)isBackedUp
